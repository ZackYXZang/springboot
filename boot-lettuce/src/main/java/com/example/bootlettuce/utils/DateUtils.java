...
public class DateUtils {
  ...
  private static Logger log = LoggerFactory.getLogger(com.example.bootlettuce.utils.DateUtils.class);
   ...
   */
  public static long getBeforeFromNow(String date, String type) {
    try {
      Date beforeDate = parseDate(date, com.example.bootlettuce.utils.DateUtils.YYYYMMDD_HMS);
      Date now = new Date();
      ...
      log.error("getBeforeFromNow error date {} , type {} ", date, type);
    }
    return 0;
  }

  /**
   * @param lastMonthNum 近lastMonthNum月
   * @return 获取最近月份
   */
  public static List<String> getLastMonths(int lastMonthNum, String pattern) {
    List<String> lastMonth = new ArrayList<>();
    LocalDate today = LocalDate.now();
    DateTimeFormatter fmt = DateTimeFormatter
        .ofPattern(StringUtils.isEmpty(pattern) ? "yyyyMM" : pattern);
